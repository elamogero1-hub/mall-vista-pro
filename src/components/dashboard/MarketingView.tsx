...
const MarketingView = ({ filters }: MarketingViewProps) => {
  ...
  if (!kpisMarketing || !kpisMarketing.tasaConversionOmnicanal) {
    return <div className="p-8 text-center">Cargando datos de marketing...</div>;
  }
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard
          ...
          value={`${kpisMarketing.tasaConversionOmnicanal?.valor || 0}%`}
          change={kpisMarketing.tasaConversionOmnicanal?.cambio || 0}
          trend="up"
        ...
        <KPICard
          ...
          value={kpisMarketing.indiceAtraccionDigital?.valor?.toFixed(2) || "0.00"}
          change={kpisMarketing.indiceAtraccionDigital?.cambio || 0}
          trend="up"
        ...
        <KPICard
          ...
          value={`S/ ${(kpisMarketing.ventasAtribuidas?.valor ? (kpisMarketing.ventasAtribuidas.valor / 1000000).toFixed(2) : "0.00")}M`}
          change={kpisMarketing.ventasAtribuidas?.cambio || 0}
          trend="up"
        ...
        <div className="glass-card p-4 flex flex-col justify-between">
            ...
            {kpisMarketing.ticketPromedioDigital && kpisMarketing.ticketPromedioOrganico ? (
            <div className="mt-4 space-y-3">
                ...
                <div>
                    ...
                    <div className="h-2 bg-secondary rounded-full overflow-hidden">
                        <div className="h-full bg-primary/50 rounded-full" style={{ width: `${(kpisMarketing.ticketPromedioOrganico.valor / (kpisMarketing.ticketPromedioDigital.valor || 1)) * 100}%` }}></div>
                    </div>
                </div>
            </div>
            ) : (
              <div className="mt-4 text-sm text-muted-foreground">Datos no disponibles</div>
            )}
            
            {kpisMarketing.ticketPromedioDigital && kpisMarketing.ticketPromedioOrganico && (
            <div className="mt-2 text-xs text-success flex items-center gap-1">
                <span className="font-medium">+{((kpisMarketing.ticketPromedioDigital.valor - kpisMarketing.ticketPromedioOrganico.valor) / (kpisMarketing.ticketPromedioOrganico.valor || 1) * 100).toFixed(1)}%</span>
